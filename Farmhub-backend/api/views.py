...
def listing_list(request):
    ...
    if amenities:
        for a in amenities.split(','):
            a = a.strip()
            if a == 'Water Access':  lands = lands.filter(has_water=True)
            if a == 'Electricity':   lands = lands.filter(has_electricity=True)
            if a == 'Road Access':   lands = lands.filter(has_road_access=True)
            if a == 'Fencing':       lands = lands.filter(has_fencing=True)
